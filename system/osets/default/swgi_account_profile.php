...
function direct_account_oset_parse_user_fullh ($f_data,$f_pageclass,$f_user_pageurl = "",$f_user_ip = "",$f_prefix = "")
{
	...
	if ((is_array ($f_data))&&(!empty ($f_data)))
	{
		...
		if ($f_data[$f_prefix."avatar_small"]) { $f_return .= "<img src='".$f_data[$f_prefix."avatar_small"]."' border='0' alt=\"".$f_data[$f_prefix."name"]."\" title=\"".$f_data[$f_prefix."name"]."\" class='pageborder{$direct_settings['theme_css_corners']}' style='float:right;padding:5px'/>"; }
		$f_return .= "<strong>";
